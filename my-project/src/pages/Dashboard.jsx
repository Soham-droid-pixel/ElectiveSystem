...
const Dashboard = () => {
  const { allotmentData, analyticsData } = useAllotment();
  const [totalStudents, setTotalStudents] = useState(0);
  const [totalCourses, setTotalCourses] = useState(0);
  const [droppedCoursesCount, setDroppedCoursesCount] = useState(0);

  useEffect(() => {
    // Calculate dynamic stats based on allotmentData
    if (allotmentData.length > 0) {
      setTotalStudents(allotmentData.length);

      const uniqueCourses = new Set(
        allotmentData.map((item) => item.allottedCourse)
      );
      setTotalCourses(uniqueCourses.size);

      const droppedCourses = allotmentData.filter(
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 px-4 py-6 sm:px-8 lg:px-16 transition-all duration-300">
        ...
        <section className="mt-8 bg-white p-6 rounded-2xl shadow-sm">
          ...
          <AllotmentTable data={allotmentData} />
        </section>
        ...
        <section className="mt-8 grid md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm">
            ...
            <CourseBarChart data={analyticsData.bar} />
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">
              Preferences Pie Chart
            </h2>
            <PieChart data={analyticsData.pie} />
          </div>
        </section>
        ...
        <section className="mt-8 bg-white p-6 rounded-2xl shadow-sm">
          ...
          <DroppedCoursesList data={allotmentData.filter(
            (item) => !item.allottedCourse
          )} />
        </section>
      </main>
